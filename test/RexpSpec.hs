...
spec = do
    ...
    describe "der" $ do
        ...
        it "derives ZERO from ONE for any character" $ do
            property $ \c -> der ONE c == ZERO

        it "derives ONE from a CHAR d for any character c if c and d are the same" $ do
            property $ \c d ->
                if c == d
                then der (CHAR c) d == ONE
                else der (CHAR c) d /= ONE

        it "derives ALT from an ALT" $ do
            property $ \c d e ->
                case der (ALT c d) e of
                    ALT _ _ -> True
                    _ -> False

        it "derives SEQ from a SEQ if the first part of the SEQ is not nullable" $ do
            property $ \c d e ->
                not (nullable c) ==> case der (SEQ c d) e of
                        SEQ _ _ -> True
                        _ -> False

        it "derives ALT from a SEQ if the first part of the SEQ is nullable" $ do
            property $ \c d e ->
                nullable c ==> case der (SEQ c d) e of
                        ALT _ _ -> True
                        _ -> False

        it "derives SEQ with a STAR at the end from STAR" $ do
            property $ \c e -> 
        ...
        it "derives a sequence of characters" $ do
            der (SEQ (CHAR 'a') (SEQ (CHAR 'b') (CHAR 'c'))) 'a' `shouldBe` SEQ ONE (SEQ (CHAR 'b') (CHAR 'c'))